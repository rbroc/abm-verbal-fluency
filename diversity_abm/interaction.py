...
class Interaction:
    ...
    def _run_trial(self, speaker, seed, turn, itr, init_seed, log=None):
        ...
        prob = [a.listen(seed, resp) 
                if a is not speaker else prob_agent
                for a in self.agents]
        metrics = []
        for a in self.agents:
            metrics += a.get_metrics(resp, self.kvals)
        if self.map_locations is not None:
        ...
        log = self._append_data(log, speaker, turn, itr, seed, init_seed,
                                resp, prob,
                                pos_init_seed, pos_response, 
                                metrics)
        return log, resp

    
    def _run_agent_loop(self, agent, log, 
                        first_seed, current_seed, n_attempts,
                        max_attempts, turn, itr, init_seed):
        ''' Runs loop for single agent, considering n-back '''    
        if (agent.matrix.data[current_seed] < self.threshold).any():
            log, current_seed = self._run_trial(agent, current_seed, 
                                                turn, itr, init_seed, log)
            success = 1
    ...
    def _append_data(self, log, agent, turn, itr, seed, init_seed, resp, 
                     prob, pos_init_seed, pos_response, metrics):
    ...
    def _get_next_agent(self, a_idx):
        ''' Get index for non-strict interactions '''
    ...
    def run_interaction(self, n_back=0, 
                        interaction_type='strict', seeds=None, 
                        return_logs=False):
        ...
        for i in range(len(self.agents)):
            cols_metrics += [*[f'resp_knnd_{k}_a{i}' for k in self.kvals], 
                             'avg_dist_remain',
        ...
        for itr in range(self.nr_sim):
            log = pd.DataFrame(columns=['agent', 'turn', 'iter', 'seed', 'response',
                                        ...
                                        *['pos_init_seed_' + str(i) 
                                          for i,_ in enumerate(self.agents)],
                                        *['pos_response_' + str(i) 
                                          for i,_ in enumerate(self.agents)],
                                        'threshold', 'nr_sim', 
                                        'max_exchanges', 'init_seed',
                                        'log_id', 'nr_agents'] + cols_metrics)
            init_seed = seeds[itr]
            ...
            if interaction_type == 'strict':
                for idx in turn_idx:
                    ...
                    while n_attempts <= n_back:
                        log, seed, n_attempts, s = self._run_agent_loop(agent, 
                                                                        log, 
                                                                        first_seed, 
                                                                        seed, 
                                                                        n_attempts,      
                ...
                for turn in range(nr_turns):
                    ...
                    first_seed = seed
                    while n_attempts <= n_back:
                        a_idx = np.argmin([np.nanmin(a.matrix.data[seed]) 
                                           for a in self.agents])
                        agent = self.agents[a_idx]
                        log, seed, n_attempts, s = self._run_agent_loop(agent, 
                                                                        log, 
                ...
                raise ValueError('''interaction_type must be strict,
